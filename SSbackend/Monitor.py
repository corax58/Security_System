...
# face_cascade = cv2.CascadeClassifier( "haarcascade_frontalface_default.xml") #xml file of the detection model

response = requests.get("http://127.0.0.1:8000/api/cameras/") # the api to get the  cameras
...
titleLabel = ctk.CTkLabel(root, text="Cameras", font=ctk.CTkFont(size=30,weight="bold"))
...
for camera in cameras:
    label = ctk.CTkLabel(scrollableframe,text=camera ,font=ctk.CTkFont(size=20), )
    label.pack(fill="x")
